...
describe("Add connection, database and collection",function(){
    it("Add a new connection",function(done){
        request("http://localhost:1234")
            .post("/add_config")
            .send({0: conn_name,1:"mongodb://localhost:27017"})
            .expect(200)
            .expect("Config successfully added", done);
    });
    
    it("Add a database",function(done){
        request("http://localhost:1234/" + conn_name)
            .post("/db_create")
...
describe("User tests",function(){
    it("Create a user",function(done){
        ...
        request("http://localhost:1234/" + conn_name)
            .post("/" + db_name + "/" + coll_name + "/user_create")
            .send(json)
            .expect(200)
    ...
    it('Delete a user', function(done){       
        request("http://localhost:1234/" + conn_name)
            .post("/" + db_name + "/" + coll_name + "/user_delete")
...
describe("Document tests",function(){
    ...
    it("Add a document",function(done){
        ...
        request("http://localhost:1234/" + conn_name)
            .post("/" + db_name + "/" + coll_name + "/insert_doc")
    ...
    it("Find our new document",function(done){
        var json = {"NewTestDocument":"Test Data"};
        var strJSON = JSON.stringify(json);
        request("http://localhost:1234/api/" + conn_name)
            .post("/" + db_name + "/" + coll_name + "/1")
            .send({"query": strJSON})
            .expect(200)
    ...
    it('Delete our new document', function(done){       
        request("http://localhost:1234/" + conn_name)
            .post("/" + db_name + "/" + coll_name + "/doc_delete")
            .send({"doc_id": doc_id})
            .expect(200)
...
describe("Remove and remove collection and connection",function(){
    it("Rename the collection",function(done){
        request("http://localhost:1234/" + conn_name)
            .post("/" + db_name + "/" + coll_name + "/coll_name_edit")
